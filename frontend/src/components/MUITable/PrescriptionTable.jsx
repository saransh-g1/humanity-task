...
export default function PrecriptionTable({ prescriptionList }) {
  ...
  const handlePayment = async (value) => {
    const apiSetQrcode = "http://localhost:8089/api/paypal/payment";
    //console.log(val);
    try {
      //console.log(token);
  ...
  const handleDownloadReceipt = async (value) => {
    ...
    const resp = await axios.get(
      `https://hmc-backend-six.vercel.app/api/prescription/invoice/${value}`,
      {
        responseType: "blob",
      }
    );
  ...
  let rows = prescriptionList.map((precription) => {
    return createData(
      precription.appointmentId.patientId.userId.firstName +
        " " +
        precription.appointmentId.patientId.userId.lastName,
      precription.appointmentId.doctorId.userId.firstName +
        " " +
        precription.appointmentId.doctorId.userId.lastName,
